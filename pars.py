        ...
        for org_data in report_list:  
            ...
            if len(etp_none_cdt_now)>0 and len(etp_cdt_last) > 0 and len(etp_cdt_now) > 0:
                ...
                if ratio_last > ratio_now:
                    a = f'Прошлый месяц - ЦДТ - {sum(publish_cdt_last)}, всего - {sum1}, процент ЦДТ - {ratio_last}%'
                    b = f'Отчетный месяц - ЦДТ -{sum(publish_cdt_now)}, всего - {sum2}, процент цдт - {ratio_now}%'
